...
if (isset($_POST["session"])) {
    session_destroy();
} else if (isset($_FILES["archivo"]) && !$_FILES["archivo"]["error"]) { // Si esta seteada la variable y no hay error procedo
    ...
    if ($uploadOk) {
        $path = "$userFolder/$name";
        if (file_exists($path)) {
            $num = 0;
            $fileName = pathinfo($name, PATHINFO_FILENAME);
            while (file_exists("$userFolder/$fileName(" . (++$num) . ")." . $extension));
            $path = "$userFolder/$fileName(" . ($num) . ")." . $extension;
        }
        $name = pathinfo($path, PATHINFO_FILENAME);
        move_uploaded_file($_FILES["archivo"]["tmp_name"], $path);
        echo "Archivo subido<br/>";
        echo "El archivo se llama $name, tiene extensión .$extension y pesa " . ($_FILES["archivo"]["size"] / 1000) . " KB.";
    }
